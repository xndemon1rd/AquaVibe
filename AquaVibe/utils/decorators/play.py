...
def PlayWrapper(command):
    async def wrapper(client, message):
        ...
        for msg in (message, message.reply_to_message) if message.reply_to_message else (message,):
            ...
            for ent in entities:
                if str(ent.type).endswith("URL"):
                    url = text[ent.offset: ent.offset + ent.length].split("&si")[0]
                    break
                if str(ent.type).endswith("TEXT_LINK") and ent.url:
                    url = ent.url.split("&si")[0]
                    break
            if url:
                break

        # Also accept a plain URL after /play or /vplay. Telegram does not
        # always attach URL entities to command arguments, so relying only on
        # entities makes direct provider links fall through to text search.
        if url is None:
            raw_args = (message.text or message.caption or "").split(maxsplit=1)
            if len(raw_args) == 2:
                candidate = raw_args[1].strip().split()[0]
                if candidate.startswith(("https://", "http://")):
                    url = candidate.split("&si")[0]
