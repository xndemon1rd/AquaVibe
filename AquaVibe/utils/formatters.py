...
def time_to_seconds(time):
    """Convert a duration such as MM:SS or HH:MM:SS to seconds.

    Some search providers return ``-`` (or an empty value) when duration is
    unavailable. Treat those values as unknown/zero instead of crashing the
    play handler.
    """
    if time is None:
        return 0
    stringt = str(time).strip()
    if not stringt or stringt in {"-", "--", "N/A", "n/a", "unknown", "Unknown"}:
        return 0
    try:
        parts = stringt.split(":")
        return sum(int(x.strip()) * 60**i for i, x in enumerate(reversed(parts)))
