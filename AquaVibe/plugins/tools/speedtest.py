...
@app.on_message(filters.command(["speedtest", "spt"]) & ADMINS)
@language
async def speedtest_function(_, message: Message, lang):
    try:
        ...
        await message.reply_text(
            "❌ Speedtest failed. The speed-test provider may be unavailable or outbound network access is restricted.\n"
            f"`{type(e).__name__}: {e}`"
        )
