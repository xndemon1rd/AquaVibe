...
from AquaVibe.utils.files import resize_file_to_sticker_size
...
BOT = (BOT_USERNAME or "aquavibe_bot").lstrip("@").lower()
BOT = "".join(ch for ch in BOT if ch.isalnum() or ch == "_") or "aquavibe_bot"
...
async def _resolve_input_user(client, user_id: int):
    """Convert Pyrogram's peer object into the InputUser required by raw sticker APIs."""
    peer = await client.resolve_peer(user_id)
    if isinstance(peer, raw.types.InputPeerUser):
        return raw.types.InputUser(user_id=peer.user_id, access_hash=peer.access_hash)
    raise PeerIdInvalid(f"Could not resolve user {user_id}")
        ...
        try:
            ...
            await client.invoke(
                raw.functions.stickers.CreateStickerSet(
                    user_id=await _resolve_input_user(client, uid),
                    title=title,
                    short_name=short,
