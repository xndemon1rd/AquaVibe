...
async def extract_media(_, message: Message):
    ...
    if len(message.command) < 2:
        return await message.reply_text("ℹ️ Use `/extract audio` or `/extract video`.", reply_to_message_id=message.id)
    ...
    try:
        ...
        if command == "audio":
            ...
            def process_video():
                subprocess.run(["ffmpeg", "-hide_banner", "-loglevel", "error", "-i", file_path, "-c", "copy", "-an", output_video], check=True, timeout=120)
