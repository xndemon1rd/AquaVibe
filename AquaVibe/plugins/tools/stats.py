...
from AquaVibe.misc import ADMINS, mongodb
from AquaVibe.plugins import ALL_MODULES
from AquaVibe.utils.database import get_served_chats, get_served_users, get_admins
from AquaVibe.utils.decorators.language import language, languageCB
from AquaVibe.utils.inline.stats import (
...
async def open_stats(client, message: Message, _):
    ...
    keyboard = build_stats_keyboard(_, is_admin)
...
async def handle_back_to_stats(client, callback_query, _):
    is_admin = callback_query.from_user and (callback_query.from_user.id in ADMINS)
    keyboard = build_stats_keyboard(_, is_admin)
...
async def handle_show_overview(client, callback_query, _):
    ...
    await callback_query.edit_message_text(caption, reply_markup=back_keyboard)
