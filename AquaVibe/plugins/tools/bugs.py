...
async def report_bug(_, msg: Message):
    ...
    # Send report to configured logger. Never let a broken/missing logger
    # chat make the /bug command fail for the user.
    if not LOGGER_ID:
        return
    ...
    try:
        ...
        # LOGGER_ID may point to a deleted/inaccessible chat (e.g. CHANNEL_INVALID).
        # The report was already acknowledged to the user, so keep the handler alive.
        print(f"[BUG REPORT] Could not send report to LOGGER_ID={LOGGER_ID}: {exc}")
