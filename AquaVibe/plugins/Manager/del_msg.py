...
from pyrogram.types import ChatPrivileges as _Priv

from AquaVibe.core.runtime import app
from AquaVibe.log_config import LOGGER as _LOGGER_FACTORY
from AquaVibe.utils.database import get_assistant
from AquaVibe.utils.permissions import is_owner_or_admin, mention
...
async def deleteall_command(client, message: Message):
    ok, owner = await is_owner_or_admin(client, message.chat.id, message.from_user.id)
    if not ok:
...
async def deleteall_callback(client, callback: CallbackQuery):
    ...
    ok, _ = await is_owner_or_admin(client, chat_id, uid)
    if not ok:
