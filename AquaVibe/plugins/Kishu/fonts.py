...
from AquaVibe.utils.font_styles import Fonts
...
async def style_buttons(c, m, cb=False):
    if cb:
        ...
        if len(m.command) < 2:
            return await m.reply_text("❌ Please provide text to style.\n\nExample: `/font Hello World!`", reply_to_message_id=m.id)
        message = m
    ...
    if cb:
        ...
        await m.reply_text(f"`{text}`", reply_markup=InlineKeyboardMarkup(buttons), reply_to_message_id=m.id)
