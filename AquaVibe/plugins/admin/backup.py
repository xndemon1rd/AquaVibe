...
from config import MONGO_DB_URI, LOGGER_ID, OWNER_ID, AUTO_BACKUP_ENABLED, DATA_EXPORT_ENABLED
from AquaVibe.log_config import LOGGER
from AquaVibe.core.dir import BACKUP_DIR
...
async def _create_backup_zip() -> str:
    ...
    zip_name = f"蒼響_Backup_{timestamp}.zip"
    ...
    with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zf:
        for root, _, files in os.walk(TEMP_DIR):
            for file in files:
                ...
                arc = os.path.join("AquaVibe", os.path.relpath(fp, TEMP_DIR))
                zf.write(fp, arc)
    ...
    while True:
        ...
        try:
            ...
            if LOGGER_ID:
                ...
                os.remove(zip_path)
                LOGGER(__name__).warning("Daily backup created but LOGGER_ID is not configured; archive discarded.")
        except Exception as e:
