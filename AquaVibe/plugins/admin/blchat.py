...
from AquaVibe.utils.database import blacklist_chat, blacklisted_chats, whitelist_chat
from AquaVibe.utils.decorators.language import language
from config import BANNED_USERS


@app.on_message(filters.command(["blchat"]) & ADMINS)
@language
async def blacklist_chat_func(client, message: Message, _):
    if not message.command or message.command[0].lower() != "blchat":
        return
...
@app.on_message(
    filters.command(["whitelistchat", "unblacklistchat", "unblchat"]) & ADMINS
)
@language
...
async def all_chats(client, message: Message, _):
    if not message.command or message.command[0].lower() not in {"blchats", "blacklistedchats"}:
        return
    text = _["black_7"]
    j = 0
