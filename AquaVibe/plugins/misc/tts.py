...
async def cmd_tts(client: Client, message: Message):
    if len(message.command) < 2:
        return await message.reply_text(
            "❌ Usage:\n`/tts <text>` — uses the default voice\n`/tts <voice_model> <text>` — choose a voice\n\nUse `/voices <text>` for guided voice selection.",
            parse_mode=ParseMode.MARKDOWN,
        )
    ...
    # Preserve the old explicit voice syntax, but also make /tts Hello work.
    if len(message.command) >= 3 and any(v["short_name"] == message.command[1] for v in _voices):
        voice = message.command[1]
        text = " ".join(message.command[2:]).strip()

    if not text:
        return await message.reply_text("❌ Please provide text to convert.")
